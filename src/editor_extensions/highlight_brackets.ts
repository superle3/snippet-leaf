...
function handleDelimitedGroup(
    ...
    const mathDelimiters = mathNode
        ? traverseTree(
              mathNode,
              new EquationText(doc.eqn, mathNode.from, mathNode.to, doc.offset),
          )
        : [];
    if (!open || !close) {
        return [];
...
function handleMathSpecialChar(
    cursor: TreeCursor,
    doc: EquationText,
): BracketResult[] {
...
type PairedBracketParent =
    | (PairedBrackets & { kind: "error_open" | "bracket" })
    | null;
    ...
    for (let i = 0; i < specs.length; i++) {
        ...
        if (pairedSpec.kind === "error_open") {
            ...
            const bracket: string =
                reverse_bracket_delimiters[pairedSpec.bracket];
            const index = findIndexReverse(
                openStack,
                (open) => open.bracket === bracket,
            );
...
function colorPairedBrackets(
    ...
    for (const eqn_info of overlays) {
            ...
            for (const spec of cached_equations[eqn_info.text]) {
                widgets.push(
                    getHighlightBracketMark(
                        spec.pos + eqn_info.overlay.from,
                        spec.className,
                        spec.bracket,
                    ),
                );
        ...
        const doc = new EquationText(
            eqn_info.text,
            eqn_info.overlay.from,
            eqn_info.overlay.to,
        );
        ...
        widgets.push(
            ...localSpecs.map((spec) =>
                getHighlightBracketMark(spec.pos, spec.className, spec.bracket),
            ),
        );
...
function highlightCursorBrackets(view: EditorView) {
    ...
    for (const range of ranges) {
        const covered_overlays = overlays.find(
            (o) =>
                o.overlay.from <= range.from &&
                o.bound.inner_end >= o.overlay.to,
        );
        if (!covered_overlays) {
            continue;
        }
        const doc = new EquationText(
            covered_overlays.text,
            covered_overlays.overlay.from,
            covered_overlays.overlay.to,
        );
        const tokens = pairBrackets(
            traverseTree(covered_overlays.bound.tree, doc),
        );
        let prev_token: null | {
