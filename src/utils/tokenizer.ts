...
const readSingleCharacterToken = (
    latexString: string,
    start: number,
): { token: Token; nextIndex: number } => {
    const end = start + 1;
...
export function* walkPairedBrackets(
    tokens: PairedBrackets[],
    depth: number = 0,
): Generator<{ region: PairedBrackets; depth: number }> {
...
export function* iterateTreeCursor(topNode: SyntaxNode, doc: EquationText) {
    ...
    while (
        cursor.node.parent &&
        cursor.node.parent.from >= doc.from &&
        cursor.parent() &&
        !NodeEquals(cursor.node, cursor.node.parent)
    ) {
        // empty
    }
    ...
    do {
        ...
        (doc.skipCursorMove || cursor.next()) &&
        cursor.from <= doc.to &&
        cursor.node !== topNode
    );
}
