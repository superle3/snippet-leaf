...
export const autoEnlargeBrackets = (view: EditorView) => {
    ...
    const covered_overlay = overlays.filter(
        (o) => o.bound.inner_start <= pos && o.bound.inner_end >= pos,
    )[0];
    if (!covered_overlay) {
        return false;
    }
    ...
    const doc = new EquationText(
        covered_overlay.text,
        covered_overlay.overlay.from,
        covered_overlay.overlay.to,
    );
    ...
    for (const { region: bracket } of walkPairedBrackets(tokens)) {
        ...
        if (
            openBracket === "{" ||
            openBracket === "\\(" ||
            openBracket.startsWith("\\left") ||
            closeBracket.startsWith("\\right")
        ) {
            continue;
        }
        ...
        if (
            settings.autoEnlargeBracketsTriggers.every(
                (trigger) => !bracketContent.includes(trigger),
            )
        ) {
            continue;
        }
