...
const rawParser = [
    ...delimiters.map(escapeRegex).sort((a, b) => a.length - b.length),
...
export const autoEnlargeBrackets = (view: EditorView) => {
    ...
    while ((match = parser.exec(text)) !== null) {
        ...
        if (isContains(sizeControls, token)) {
            ...
            if (skipNext === null || /\S/.test(text.slice(skipNext, index))) {
                stack.push({
                    index,
                    match: token,
                    level: scopeStack[scopeStack.length - 1],
                });
        ...
        const skipCurrent =
            skipNext !== null && !/\S/.test(text.slice(skipNext, index));
        skipNext = null;
        const expectedOpen = closeToOpen[token];
        for (let i = stack.length - 1; i >= 0; i--) {
            const openMatch = stack[i];
            if (
                openMatch.match === expectedOpen &&
                ("ignore" in openMatch || skipCurrent)
            ) {
                stack.splice(i, 1);
                break;
            }
